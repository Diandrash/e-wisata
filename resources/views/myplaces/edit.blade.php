...
@section('container')
    <section class="py-20 mx-4 lg:mx-10">
        <h1 class="font-semibold text-2xl">Edit Wisata</h1>

        <form action="/myplaces/{{ $place->id }}/edit" method="post" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            ...
            <div class="form-area mt-5">
                ...
                <input required type="text" name="name" class="h-14 w-full font-semibold text-lg pl-3 rounded-md mt-2" placeholder="Gunung Rinjani" value="{{ old('name', $place->name) }}">
                @error('name')
            ...
            <div class="form-area mt-3">
                ...
                <select name="category_id" class="font-semibold w-full h-14 text-base pl-3 mt-2" id="">
                    ...
                    @foreach ($categories as $category)
                        <option value="{{ $category->id }}" @if ($place->category_id == $category->id)
                            selected
            ...
            <div class="form-area mt-5">
                ...
                <textarea name="description" class="w-full font-semibold text-lg rounded-md mt-2 p-3" id="" cols="30" rows="10" placeholder="Deskripsi Wisata disini">{{ old('description', $place->description) }}</textarea>
                @error('description')
                    <h1 class="mt-2 text-red-600">{{ $message }}</h1>
                @enderror
            </div>
            ...
            <div class="form-area mt-5">
                ...
                <input required type="number" name="price" class="h-14 w-full font-semibold text-lg pl-3 rounded-md mt-2" placeholder="198000" value="{{ old('price', $place->price) }}">
                @error('price')
                    <h1 class="mt-2 text-red-600">{{ $message }}</h1>
                @enderror
            </div>
            ...
            <div class="form-area mt-5">
                ...
                <img src="{{ '/placeimages/' . $place->image }}" alt="" class="w-56 mt-1 mb-3">
                <label for="description" class="font-semibold text-lg">Edit Gambar Wisata</label>
